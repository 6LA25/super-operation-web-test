<template>
  <v-app>
    ...
    <v-content class="divder pb-12" :style="background">
      <v-expand-transition>
        <v-tabs color="secondary" style="position: sticky; top: 64px;z-index: 5" v-show="tabsView && tabList.length"
                show-arrows>
          <v-tab :name="i" @contextmenu="showMenu" v-for="(item, i) in tabList" :key="item.name" :to="item.path">
            {{ item.title }}
            <v-icon size="20" v-if="item.title!='首页'" @click.stop.prevent="closeTab(i)" @contextmenu.stop.prevent="">
              mdi-close
            </v-icon>
          </v-tab>
        </v-tabs>
    ...
    <v-navigation-drawer width="300" v-model="setting" overlay-opacity="0" floating temporary right app>
      <v-sheet class="ma-4" color="transparent">
        ...
        <v-color-picker width="268" canvas-height="100" class="mb-3" v-model="primarySet" hide-inputs flat
                        dark></v-color-picker>
        <v-flex class="mb-3 subtitle-1">关键颜色</v-flex>
        <v-color-picker width="268" canvas-height="100" class="mb-3" v-model="secondarySet" hide-inputs flat
                        dark></v-color-picker>
